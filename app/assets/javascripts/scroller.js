...
MYAPP.FixedInfoScroller = function (options) {
	...
	my.init = function() {
		...
		view.bind("scroll resize", function() {
			...
			var relativeToElementBottom = relativeToElement.offset().top + relativeToElement.height() + 15;
			var relativeToRight = parseInt(relativeToElementMargin);

			// Get the current scroll of the window.
			var viewTop = view.scrollTop() + 80;
			var fixedElementBottom = fixedElement.offset().top + fixedElement.height();

			// Get the current left offset for FixedElement
			var fixedElementLeft = (($(document).width() - 940) / 2) + 20
